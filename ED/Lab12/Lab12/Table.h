...
public:
	...
	//------------------------------------------------------
	void deposit(std::string p, std::string a, int m) {
		if (exists(p)) {
			if (get(p).exists(a)) 
				get(p).insert(a, m + get(p).get(a));
			else 
				get(p).insert(a, m);
		}
	...
	void transfer(std::string p1, std::string a1, std::string p2, std::string a2, int m) {
		if (get(p1).get(a1) >= m) {
			if (exists(p2)) {
				if (get(p2).exists(a2)) 
					get(p2).insert(a2, m + get(p2).get(a2));
				else 
					get(p2).insert(a2, m);
			}
			else {
			...
			get(p1).insert(a1, get(p1).get(a1) - m);
		}
	}

	int money(std::string p) {
		int sum = 0;
		V accountTable = get(p);
		typename V::Iterator it = accountTable.begin();
		while (it != accountTable.end()) {
			sum += it.value();
			it.next();
		}
		return sum;
