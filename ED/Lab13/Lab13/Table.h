...
public:
	...
	//------------------------------------------------------
	void addCase(std::string ID, std::string name, std::string type, int prioritym) {
		V table_1 = V();
		V table_2 = V();
		table_1.insert(type, prioritym);
		table_2.insert(name, table_1);
		insert(ID, table_2);
	}

	void getCase(std::string ID) {
		if (exists(ID)) {
			return get(ID)
		}
		else
			return "This case does not exist";
	}
